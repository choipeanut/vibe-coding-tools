...
> ⛔ **절대 규칙 — 이걸 어기면 결과물은 실패다:**
> `steps[].prompt`의 **원본 텍스트를 그대로 출력하는 것은 금지**한다.
> "ㅇㅇ", "dd\\ㅇㅇ", "여기 있는 md에 맞춰서 개발해", "흠 안되는디" 같은
> 캐주얼·승인·짧은 문구를 **단 하나도 그대로 쓰지 마라.**
> 출력 전에 모든 `프롬프트:` 줄을 점검해서, 원본 그대로인 게 하나라도 있으면
> 다시 작성한다. 모든 프롬프트는 아래 규칙대로 **반드시 재작성된 상태**여야 한다.

### 작성 규칙

**1. 프롬프트 다듬기 (필수)**: 각 step의 원본 프롬프트는 버리고, **그 단계에서 실제로 일어난 일 + 변경된 파일(`files_changed`)**을 근거로 **전문 개발자가 처음부터 작성했을 법한 구체적이고 명확한 지시문**으로 100% 재작성한다. 원본이 "ㅇㅇ"처럼 내용이 없으면, 그 단계에서 실제 바뀐 파일을 보고 "무엇을 시켰을지"를 역으로 추론해 작성한다.

**변환 예시 (반드시 이 수준으로):**
| 원본 (그대로 쓰면 실패) | 재작성 (이렇게 써야 함) |
|---|---|
| `"ㅇㅇ"` (blog.md 변경됨) | `"M1 초안 품질 확인 완료. /blog 커맨드에 Notion 업로드 단계를 추가해줘."` |
| `"여기 있는 md에 맞춰서 개발해."` | `"AGENTS.md, TASKS.md 명세를 읽고 T1부터 순서대로 구현해줘. stdlib만 쓰고 태스크마다 검증해줘."` |
| `"흠 안되는디"` (디버그/수정 발생) | `"/blog 실행 시 명령이 인식되지 않는 원인을 로그로 분석하고 수정해줘."` |

원본 프롬프트의 표현·맞춤법·구어체는 **결과물에 절대 노출되지 않는다.**
